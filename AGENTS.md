...
- SPIFFS config payloads live in `config/` (with templates in `config/`); Wokwi diagrams in `wokwi/`; docs and examples in `docs/`; generated outputs in `build/`.
...
- SPIFFS-only push: `mkspiffs -c config -s 0x30000 build/config.bin; esptool write_flash 0x610000 build/config.bin`.
...
- Use clear test names mirroring the behavior under test; keep payload samples under `config/` or dedicated fixtures instead of inline strings.
...
- Never commit real MQTT credentials, certificates, or device configs; keep samples under `config/` and load real values into SPIFFS locally.
