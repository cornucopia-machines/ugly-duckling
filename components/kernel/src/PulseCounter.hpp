...
// ============================================================================
// UlpPulseCounter — RTC GPIO path, no per-pulse CPU wakeups
// ============================================================================

/**
 * @brief Pulse counter backed by the ULP-RISC-V coprocessor.
 *
 * The ULP polls the GPIO continuously at ~8 MHz regardless of main CPU sleep state.
 ...
 *
 * Requires an RTC-capable GPIO (GPIO 0–21 on ESP32-S3).
 ...
 *
 * For RTC-capable GPIOs (GPIO 0–21 on ESP32-S3): creates a UlpPulseCounter.
 * For all other GPIOs: creates a GpioPulseCounter (interrupt-based, light-sleep aware).
 *
 * Call start() after all counters have been created to load and start the ULP firmware.
...
public:
    ...
    /**
     * @brief Load and start the ULP firmware.
     *
     * Must be called after all ULP channels are registered (i.e., after all create() calls
     * for RTC-capable GPIOs). Safe to call even if no ULP channels were created (no-op).
...
private:
    ...
    std::shared_ptr<PulseCounter> createUlp(const PulseCounterConfig& config) {
        ...
        // Route pin to RTC domain with pull-down.
        ...
        // Debounce in ULP CPU cycles: ULP-RISC-V runs at ~8 MHz (8 cycles per microsecond).
        uint32_t debounceCycles = static_cast<uint32_t>(config.debounceTime.count()) * 8u;
        ulp_gpio_num[index]        = static_cast<uint32_t>(gpio);
        ulp_debounce_cycles[index] = debounceCycles;
        ulp_pulse_count[index]     = 0;

        LOGTD(PULSE, "Registered ULP pulse counter on pin %s (channel %" PRIu32 ", debounce %" PRIu32 " cycles)",
            config.pin->getName().c_str(), index, debounceCycles);
