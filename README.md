...
- ESP-IDF v5.5.2 (see [installation instructions](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/get-started/index.html))
...
#### Uploading just config files

To upload only the SPIFFS image:

```bash
mkspiffs -c config -s 0x30000 build/config.bin; esptool write_flash 0x610000 build/config.bin
```
